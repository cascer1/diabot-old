...
let app = new Clapp.App({
    ...
    onReply: (msg, context) => {
        ...
        let emoji = christmas_emoji[Math.floor(Math.random() * christmas_emoji.length)];

        if (functions.isChristmas()) reply = "\n" + msg + "\n" + emoji;

        context.msg.reply(reply)
            .then(bot_response => {
                if (cfg.deleteAfterReply.enabled) {
                    context.msg.delete(cfg.deletePromptAfterReply.time)
                        .then(msg => console.log(`Deleted message from ${msg.author}`))
                        .catch(console.log);
                    bot_response.delete(cfg.deleteReplyAfterReply.time)
                        .then(msg => console.log(`Deleted message from ${msg.author}`))
                        .catch(console.log);
                } else {
                    // User doesn't want us to specifically remove all bot messages
                    // Only remove messages that are explicitly marked for removal
                    let commandName = functions.getCommand(context.msg.content, app);
...
console.log(JSON.stringify(app.commands, 2, ' '));
