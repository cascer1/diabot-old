...
let commands       = require("../global/commands");
    ...
    if (options.cleanup) {
        ...
        try {
            bot.user.setGame("Shutting Down")
                .then(() => {
                    console.log("Set status to 'Shutting Down'");
                    console.log("Stopping queue writer");
                    rabbit.shutdown()
                        .then(console.log("Stopped queue writer"))
                        .then(console.log("Disconnecting from Discord"))
                        .then(() => {
                            bot.destroy()
                                .then(process.exit(0));
                        });
...
function shutdownHandler(options) {
    if (options.crash) {
        console.log("Crashed, starting shutdown");
    }

    if (options.cleanup || options.crash || options.exit) {
        try {
                    ...
                    bot.user.setGame("Shutting Down")
                        .then(() => {
                            ...
                            rabbit.shutdown()
                                .then(console.log("Stopped queue writer"))
                                .then(console.log("Disconnecting from Discord"))
                                .then(bot.destroy().then(resolve("All done, good bye")))
                                .done(resolve("All done, good bye"));
                        })
                        .catch((error) => {
                            console.error(error);
                            reject(error);
...
// Graceful.on("exit", () => {
//     return shutdownHandler({cleanup: true})
//         .then(console.log("Shutdown complete"));
// });
//
// Graceful.on("uncaughtException", () => {
//     return shutdownHandler({crash: true})
//         .then(console.log("Shutdown complete"));
// });