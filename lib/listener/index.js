...
rabbit.onUnhandled((message) => {
    ...
    let channelId = message.properties.headers.channel;
    let messageId = message.properties.headers.message;
    ...
    if (options.cleanup) {
        try {
            ...
            bot.user.setGame("Shutting Down")
               .then(rabbit.shutdown())
               .then(bot.destroy())
               .then(console.log("All Done, good bye"));
        }
        catch (err) {
            console.error(err);
        }
